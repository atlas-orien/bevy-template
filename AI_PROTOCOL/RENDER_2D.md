...
- `.frames.ron` 的 Bevy `AssetLoader` 属于 `frame_animation`；它使用 `helper::assets::manifests::FrameManifest` 作为标准文件格式，并使用 `helper::assets::ron` 做薄反序列化，但转换成 `FrameAnimationManifest2d` 的逻辑不移出 `render_2d`。
