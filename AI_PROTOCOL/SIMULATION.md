...
`crates/simulation` 是游戏 runtime 语义层，负责游戏流程和世界调度。
...
- runtime/session 进入调度：写到 `crates/simulation/src/spawning`。
- 清理策略：写到 `crates/simulation/src/cleanup`。
- 关卡、回合、runtime session 生命周期：写到 `crates/simulation/src/lifecycle`。
...
- `simulation` 可以依赖 `prefab`，用于 runtime setup 中使用封装好的对象模板和 runtime 能力。
- `simulation` 可以依赖 `input` 和 `intent`，并作为唯一游戏 runtime 负责注册和调度它们。
- `simulation` 必须依赖 `error`。
- `simulation` 不依赖 `ecs`。
- `simulation` 不依赖 `render_2d` 或 `render_3d`。
- `simulation` 不直接依赖 `physics`；对象组合通过 `prefab` 完成，并由 `simulation` 注册 `PrefabPlugin`。
