...
- `keyboard`: 键盘输入适配。
  - `keyboard/bindings`: 按功能拆分的默认键位配置。
- `mouse`: 鼠标输入适配。
- `gamepad`: 手柄输入适配。
...
```text
keyboard/bindings/ui.rs        // UI 菜单、按钮焦点
keyboard/bindings/gameplay.rs  // 角色场景、交互、暂停
```

未来新增背包、对话、调试控制台等输入时，优先新增对应 binding 文件，而不是把所有按键塞进一个巨大列表。
