...
use crate::{
    PhysicsCollider2d, PhysicsPointProjection2d, PhysicsQueryFilter, PhysicsRayHit2d,
    PhysicsShapeCastHit2d, PhysicsShapeCastHitDetails2d,
};

use super::convert;

#[derive(SystemParam)]
pub struct PhysicsQuery2d<'w, 's> {
    context: ReadRapierContext<'w, 's>,
}

impl PhysicsQuery2d<'_, '_> {
    pub fn cast_ray(
        &self,
        origin: Vec2,
        direction: Vec2,
    ...
    pub fn intersect_point(&self, point: Vec2, filter: PhysicsQueryFilter) -> Vec<Entity> {
        let Ok(context) = self.context.single() else {
            return Vec::new();
        };
        let mut hits = Vec::new();
    ...
    pub fn cast_shape(
        ...
        Some(PhysicsShapeCastHit2d {
            entity,
            time_of_impact: hit.time_of_impact,
            details: hit.details.map(|details| PhysicsShapeCastHitDetails2d {
