...
use crate::{
    PhysicsCollisionEnded, PhysicsCollisionStarted, PhysicsContactForce3d, PhysicsSensorMarker,
...
pub fn forward_contact_force_events(
    ...
    mut force_events: MessageWriter<PhysicsContactForce3d>,
) {
    for event in rapier_events.read() {
        force_events.write(PhysicsContactForce3d {
            a: event.collider1,
            b: event.collider2,
