...
推荐：marker 组件 `DemoPlayerControlledMarker` 定义在
...
1. `crates/ecs/src/components/characters/`：`DemoPlayerControlledMarker` marker。
...
3. `crates/prefab/src/world_2d/characters/`（目前是空文件）：`demo_player.rs`：
   `DemoPlayerPrefab`，组合：
   - ecs：`GameplayEntityId`、`DemoPlayerControlledMarker`、`MovementIntent`、`Speed`、
...
当前 demo 地面横向宽度约 3072px，并在 `x=-900/0/900` 放了三根彩色
`DemoLandmark` 标记柱。测试相机跟随时，从 Start 进入场景后按住 A 或 D：
角色会保持在镜头附近，彩色标记柱、地面和背景会相对屏幕移动；这就是 follow
camera 与视差背景同时生效的可见参照。
...
2. `crates/prefab/src/world_2d/demo_level/`：`sensor_zone.rs`：
   ...
   `GameplayEntityId` + 一个 ecs marker `DemoSensorZoneMarker`
...
4. 事件链：physics 的碰撞事件（`crates/physics/src/events/collision.rs`）→
   ...
   - **推荐**：`crates/ecs/src/systems/interaction/`（已预留）写系统消费
     ...
     `demo_sensor_bridge_system`：physics 碰撞事件 + `DemoSensorZoneMarker` marker →
